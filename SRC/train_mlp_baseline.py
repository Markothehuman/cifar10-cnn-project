...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train an MLP baseline on CIFAR-10.")
...
def resolve_output_dir() -> Path:
    ...
    candidates = [
        PROJECT_ROOT / "Outputs" / "baseline_mlp" / run_name,
        PROJECT_ROOT / "Data" / "baseline_mlp_outputs" / run_name,
    ]
    ...
    raise PermissionError("Could not create an output directory for training artifacts.")
...
def save_training_plot(history: list[dict], summary: dict, output_dir: Path) -> Path | None:
    ...
    axes[0].set_title("MLP Baseline Loss")
    ...
    axes[1].set_title("MLP Baseline Accuracy")
...
def main() -> None:
    ...
    model = MLPBaseline(dropout=args.dropout, num_classes=len(CIFAR10_CLASSES)).to(device)
    ...
    best_model_path = output_dir / "best_mlp_baseline.pt"
